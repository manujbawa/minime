...
export class DatabaseService {
     ...
     */
    async createSession(projectId, sessionName, sessionType = 'mixed', description = null, metadata = {}) {
        const result = await this.query(`
            INSERT INTO sessions (project_id, session_name, session_type, description, metadata)
            VALUES ($1, $2, $3, $4, $5)
            RETURNING id, project_id, session_name, session_type, description, metadata, is_active, created_at, updated_at
        `, [projectId, sessionName, sessionType, description, JSON.stringify(metadata)]);
        
        this.logger.info(`Created session: ${sessionName}`, { 
    ...
    async getMemoryById(id) {
        const result = await this.query(`
            SELECT 
                ...
                m.summary, m.importance_score, m.tags, m.metadata, 
    ...
    async updateMemory(id, updates) {
        ...
        const allowedUpdates = ['content', 'memory_type', 'summary', 'importance_score', 'tags', 'metadata'];
